...
class ResPFNN(PFNN):
    ...
    def __init__(self, nControlPoints, input_dim, output_dim, style_dim, dropout_rate, batchsize, name='ResPFNN', sess=None):
        ...
        self.style_W0 = tf.get_variable(name="s_W0", shape=(self.style_dim, self.hidden_dim), initializer=tf.zeros_initializer())
        self.style_W1 = tf.get_variable(name="s_W1", shape=(self.hidden_dim, self.style_dim), initializer=tf.zeros_initializer())
        self.style_b1 = tf.get_variable(name="s_b1", shape=(self.hidden_dim,), initializer=tf.zeros_initializer())
    ...
    def create_model_diagonal(self):
        ...
        t1 = tf.matmul(tf.squeeze(H1, -1), tf.transpose(self.style_W0))
        t2 = tf.matmul(t1, tf.diag(self.style_diagonal))

        # print("style_w1 shape: ", self.style_W1.shape)
        # style_residual = tf.matmul(self.style_W1, t2) + self.style_b1
        style_residual = tf.matmul(t2, tf.transpose(self.style_W1)) + self.style_b1
    ...
    def create_model(self):
        ...
        style_residual = tf.matmul(tf.matmul(tf.squeeze(H1, -1), tf.transpose(self.style_W0)), tf.transpose(self.style_W1)) + self.style_b1
    ...
    def style_fine_turning(self, training_data, output_data, n_epoches, learning_rate=0.0001):
        ...
        self.sess.run(tf.variables_initializer(self.optimizer.variables() + self.style_params))
        n_samples, n_features = training_data.shape
        last_mean = 0
        for epoch in range(n_epoches):
            batchinds = np.arange(n_samples // self.batchsize)
